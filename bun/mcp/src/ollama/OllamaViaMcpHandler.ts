...
import type { IOllamaViaMcpConfig } from "./IOllamaViaMcpConfig";
import { Client } from "@modelcontextprotocol/sdk/client";
import { StreamableHTTPClientTransport } from "@modelcontextprotocol/sdk/client/streamableHttp.js";

export class OllamaViaMcpHandler {

  private readonly _tmap: Map<string, ICoreTool<ZodTypeAny, ZodTypeAny>> = new Map();

  constructor(
    private readonly _c: IOllamaViaMcpConfig,
  ) {}
  ...
  async callOllama(taskDescription: string): Promise<string | undefined> {
    ...
    console.log(`OllamaViaMcpHandler: Requesting initial response...`);
	...
	if (coreTool !== undefined) {
	  console.log(`OllamaViaMcpHandler: Tool "${requestedToolName}" requested by model.`);
	  ...
	  console.log(`OllamaViaMcpHandler: Connecting to MCP server at ${mcpServerHostUrl.href} ...`);
	  await client.connect(transport);
	  console.log("OllamaViaMcpHandler: Connected.");

	  console.log(`OllamaViaMcpHandler: Calling tool "${coreTool.toolName}" via MCP.`);
	  ...
	  console.log(`OllamaViaMcpHandler Result: ${JSON.stringify(toolOutput)}`);
	  client.close();
	  console.log("OllamaViaMcpHandler: MCP Client closed.");

	  console.log(`OllamaViaMcpHandler: Passing tool response to Ollama model...`);
	  ...
	  throw new Error(`OllamaViaMcpHandler: Unexpected tool call: ${call.function.name}`);
	}
      }
    }
    else {
